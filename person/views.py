...
from press.models import DistributionPartyMembers, Sympathizer, DistributionSympathizerMember
import datetime
...
def profile(request: HttpRequest):
    if request.user.party_member:
        ...
        sympathizer = Sympathizer.objects.filter(name=f"{request.user.last_name} {request.user.first_name}").first()
        my_distribution = DistributionSympathizerMember.objects.select_related('distribution') \
            .filter(member_id=sympathizer.pk) \
            .filter(distribution__distribution_date__month=datetime.date.today().month) \
            .filter(distribution__distribution_date__year=datetime.date.today().year) \
            .order_by('distribution__distribution_date') \
            .all()
        all_distribution = DistributionSympathizerMember.objects.select_related('distribution') \
            .filter(member_id=sympathizer.pk) \
            .order_by('distribution__distribution_date') \
            .all()
